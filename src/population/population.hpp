...
template<typename T>
Population<T>::Population(const std::vector<T>& initial_genes, u32 seed) : size(initial_genes.size()), genes(initial_genes), generator(seed) {}

template<typename T>
Population<T>::Population(int size, const std::function<T(std::mt19937&)>& initialize, u32 seed) : size(size), genes(size), generator(seed) {
